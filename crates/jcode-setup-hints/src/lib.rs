...
fn install_niri_launch_hotkeys() -> Result<bool> {
    ...
    // Write the launch hotkeys to a standalone file that the main config
    // `include`s, instead of splicing into the first `binds { }` block of
    // config.kdl. Splicing into the first block is fragile: the first `binds {
    // }` in a real config is often a nested block (e.g. `recent-windows`) that
    // only accepts `next-window`/`previous-window`, which makes the injected
    // `spawn` binds invalid KDL and breaks niri's config parse on every launch.
    // A dedicated `dms/jcode-binds.kdl` file keeps jcode's binds valid and
    // removable regardless of how the user structures their config.
    let inc_path = niri_jcode_binds_path(&config_path);

    let inc_text = format!(
        "// Auto-generated by jcode — niri launch hotkeys (managed).\n\
         // Edit in ~/.jcode config; do not hand-edit.\n\
         binds {{\n{block}\n}}\n"
    );

    let mut changed = false;
    if !inc_path.exists() || std::fs::read_to_string(&inc_path).map(|s| s != inc_text).unwrap_or(true)
    {
        backup_compositor_config(&inc_path);
        storage::write_bytes(&inc_path, inc_text.as_bytes())
            .with_context(|| format!("writing {}", inc_path.display()))?;
        changed = true;
    }

    // Ensure config.kdl references the include file (and drop any legacy inline
    // managed block that a previous jcode version spliced into the wrong
    // `binds { }` block, which would otherwise keep config.kdl invalid).
    let current = std::fs::read_to_string(&config_path)
        .with_context(|| format!("reading {}", config_path.display()))?;
    let cleaned = linux_niri::strip_managed_block(&current);
    let inc_ref = format!("include \"{}\"", inc_relative_to_config(&config_path, &inc_path));
    let needs_include = !cleaned.contains(&inc_ref);
    if cleaned != current || needs_include {
        ...
        if needs_include {
            text.push_str(&inc_ref);
            text.push('\n');
        }
        storage::write_bytes(&config_path, text.as_bytes())
...
/// Path of the dedicated jcode launch-hotkey include file, next to config.kdl.
#[cfg(any(test, target_os = "linux"))]
fn niri_jcode_binds_path(config_path: &std::path::Path) -> std::path::PathBuf {
    let parent = config_path
        .parent()
        .map(ToOwned::to_owned)
        .unwrap_or_else(|| std::path::PathBuf::from("."));
    parent.join("dms").join("jcode-binds.kdl")
}

/// Compute the include path to reference `inc` from `config_path`'s directory,
/// using `/` separators as niri's `include` expects a config-relative path.
#[cfg(any(test, target_os = "linux"))]
fn inc_relative_to_config(config_path: &std::path::Path, inc: &std::path::Path) -> String {
    let config_dir = config_path.parent().unwrap_or_else(|| std::path::Path::new("."));
    let rel = inc.strip_prefix(config_dir).unwrap_or(inc);
